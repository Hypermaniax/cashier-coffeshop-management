...
export const productService = {
  async createProduct(data: CreateProductDto) {
    ...
    const uploadedMenuImageUrl =
      data?.image
        ? await storageService.saveProductImage(data?.image as File)
        : null;
  ...
  async updateProduct(id: string, data: UpdateProductDto) {
    ...
    if (data?.image && typeof data.image !== "string") {
      const uploadedMenuImageUrl = await storageService.saveProductImage(data.image as File);
      data.image = uploadedMenuImageUrl;

      if (product.image) {
        await storageService.deleteProductImage(product.image).catch((err) =>
